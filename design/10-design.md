...
| [`src/cli.ts`](../src/cli.ts)                   | Flags, the precedence ladder, and every decision about which mode a site is in | `adapter`, `generic`, `data`, `changelog`, `merge`, `staticServer`, types from `index` |

The graph is acyclic at runtime. It has two apparent cycles, both the same
shape — `index` imports `route` for values while `route` imports `index` for
types, and `index` re-exports `data` for values while `data` imports `index` for
types. Both are erased at compilation and so are not cycles in anything that
executes. Each is acceptable only while the arrow stays type-only in that
direction; the moment `route` or `data` needed a _value_ from `index`, the shared
declarations would have to move below all three rather than the import being
permitted.
...
| ---------------------------------------------------- | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Filesystem — reads, a declared stylesheet            | A site-wide stylesheet is unreadable or resolves outside the root                   | An explicit check before any write                                           | Ends the build naming the declaration                                                                                                                | None: every declared stylesheet is read and contained before anything is written (**C16**)                                                   |
| Filesystem — reads, a generic asset or Markdown file | A local asset or Markdown reference is absent, broken, or resolves outside the root | The walk over the document's links, which copies each asset as it reaches it | Ends the build naming the declaration and the document it is in                                                                                      | A partially written output tree, already cleared of its predecessor. **C16**'s ordering is a stylesheet guarantee and does not extend here** |
